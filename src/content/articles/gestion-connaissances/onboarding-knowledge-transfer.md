...
Sur le volet qualité-méthodes, l'identification puis la correction de 12 processus commerciaux génère un gain de +15 points NPS (Net Promoter Score, score de propension à recommander) interne — mais ce gain ne tient que parce que les corrections ont été intégrées aux scripts CRM (Customer Relationship Management, gestion de la relation client) et aux fiches réflexes utilisées au quotidien, pas écrites dans un référentiel parallèle. Documentation vivante, encore une fois.
