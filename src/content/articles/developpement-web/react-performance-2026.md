...
Chez ACTIV PARTNERS, où j'ai piloté la refonte de 8 sites WordPress avec architecture React entre 2020 et 2021, on passait un temps fou à mémoïser à la main — `useMemo` sur les listings filtrés, `useCallback` sur chaque handler passé à un enfant, `React.memo` sur les Row d'un tableau. C'était l'hygiène React 16/17 de l'époque. Les +40 % de visibilité organique obtenus reposaient autant sur les Core Web Vitals (LCP, INP) que sur le balisage et Yoast/Rank Math — et la mémoïsation manuelle pesait sur la lisibilité sans changer grand-chose en bout de chaîne. Avec React Compiler aujourd'hui, l'essentiel de cette charge cognitive disparaît : sur les composants de cards et de filtres qu'on rebrassait à chaque session de code review, ce sont littéralement plusieurs dizaines d'hooks par fichier qui deviennent inutiles.
