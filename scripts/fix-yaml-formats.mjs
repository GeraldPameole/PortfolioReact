...
function fixYamlFormats(filePath) {
  try {
    const content = fs.readFileSync(filePath, "utf8");

    // Fix the specific format issues
  ...
  try {
    ...
    for (const file of files) {
      if (file.endsWith(".md")) {
        ...
        if (fixYamlFormats(filePath)) {
          fixedCount++;
          console.log(`Fixed YAML formats in: ${file}`);
        }
      }
    }

    console.log(`Fixed YAML formats in ${fixedCount} files.`);
  } catch (error) {
