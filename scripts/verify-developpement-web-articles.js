...
const articlesDir = path.join(__dirname, '../src/content/articles/developpement-web');

// Structure standard requise
...
function analyzeArticle(filePath) {
  ...
  REQUIRED_SECTIONS.forEach(section => {
    if (section === 'Introduction') {
      if (!body.includes('## Introduction')) {
        missingSections.push('Introduction');
      } else {
        presentSections.push('Introduction');
      }
    } else {
  ...
  let sourceCount = 0;
  if (sourcesSection) {
    // Compter les sources (lignes avec - ou [^)
  ...
  const section6Index = body.indexOf('## 6. SOURCES ET RÉFÉRENCES');
  const section7Index = body.indexOf('## 7. ARTICLES ANNEXES');
  
  if (section1Index !== -1) sectionsOrder.push({ name: '1. FONDAMENTAUX', index: section1Index });
  if (section2Index !== -1) sectionsOrder.push({ name: '2. ANALYSE', index: section2Index });
  if (section6Index !== -1) sectionsOrder.push({ name: '6. SOURCES', index: section6Index });
  if (section7Index !== -1) sectionsOrder.push({ name: '7. ANNEXES', index: section7Index });
  
  sectionsOrder.sort((a, b) => a.index - b.index);
  
  // Vérifier l'ordre correct
  const correctOrder = sectionsOrder.map(s => s.name);
  const expectedOrder = ['1. FONDAMENTAUX', '2. ANALYSE', '6. SOURCES', '7. ANNEXES'];
  const isOrderCorrect = correctOrder.join(',') === expectedOrder.join(',');
  
  // Vérifier les tableaux
  const tableCount = (body.match(/\|/g) || []).length / 2; // Approximatif
  const hasTableHeaders = body.includes('|') && body.includes('---');
  ...
  if (!isOrderCorrect) {
    issues.push(`Ordre des sections incorrect: ${correctOrder.join(' -> ')}`);
  }
...
console.log('RAPPORT DE VÉRIFICATION - DOMAINE DÉVELOPPEMENT-WEB');
console.log('='.repeat(80));
...
results.forEach(result => {
  ...
  if (result.stats.sections.present !== result.stats.sections.total) {
    console.log(`   Sections: ${result.stats.sections.present}/${result.stats.sections.total}`);
  }
  ...
  console.log(`   Tableaux: ${result.stats.tables}`);
...
console.log(`\nArticles avec avertissements: ${articlesWithWarnings.length}`);
if (articlesWithWarnings.length > 0) {
  console.log('\nArticles à améliorer:');
  articlesWithWarnings.forEach(r => {
    console.log(`  - ${r.fileName}: ${r.warnings.join(', ')}`);
  });
}

// Sauvegarder le rapport
const reportPath = path.join(__dirname, '../RAPPORT_VERIFICATION_DEVELOPPEMENT_WEB.json');
