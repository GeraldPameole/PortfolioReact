...
function fixLintingErrors(content) {
  ...
  // Convertir **Titre** en #### Titre si c'est au début d'une ligne
  content = content.replace(/^\*\*([^*]+)\*\*\s*$/gm, (match, title) => {
    // Vérifier si c'est vraiment un titre et pas juste du texte en gras
    if (title.length < 60 && !title.includes('.') && !title.includes(',')) {
      return `#### ${title}`;
    }
    return match;
  });
  
  // MD024: Duplicate headings - renommer les doublons
  const headingCounts = {};
...
console.log(`🔧 Correction des erreurs de linting dans ${articles.length} articles...\n`);
